...
<template>
    <div class="flex items-center justify-between">
        <div v-if="label">
            ...
            <p v-if="description" class="mt-0.5 text-xs text-slate-500 dark:text-gray-400">
                {{ description }}
            </p>
        </div>
        ...
        <button
            ...
            :class="modelValue ? 'bg-primary' : 'bg-slate-300 dark:bg-white/20'"
        >
            <span
                class="inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform duration-200 dark:bg-secondary"
